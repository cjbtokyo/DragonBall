...
switch($_GET['act']){
	case 'nowa':
		if(!empty($_POST['do']) && !empty($_POST['tytul']) && !empty($_POST['tresc'])){
			...
			$do_gracz_id = mysql_fetch_array(mysql_query("select gracz from dragonball_gracze where login = '".$do."'"));
			

			if(empty($do_gracz_id)) echo "<p class='error'>nie ma takiego gracza</p><br class='clear'>"; else {
				mysql_query($q="
				insert into dragonball_poczta (od, do, typ, tytul, tresc, data) 
				values 
	...
	case 'wyslane':
		if(!empty($_GET['usun'])){
			...
			$del = mysql_query("delete from dragonball_poczta where id = ".$_GET['usun']." and od = ".$uzytkownik['gracz']." and typ = 2");
		...
		$poczta = mysql_query("select p.*, login from dragonball_poczta p inner join dragonball_gracze on p.do = gracz where p.od = ".$uzytkownik['gracz']." and typ = 2 order by p.id desc");
	...
	default:
		...
		if(!empty($_GET['usun'])){
			$_GET['usun'] = (int)$_GET['usun'];
			$del = mysql_query("delete from dragonball_poczta where id = ".$_GET['usun']." and do = ".$uzytkownik['gracz']." and typ = 1");
			if(mysql_affected_rows() == 1) echo "<p class='note'>usunięto wiadomość</p><br class='clear'>"; else echo "<p class='error'>nie ma takiej wiadomości</p><br class='clear'>";
		}
		$poczta = mysql_query("select p.*, login from dragonball_poczta p inner join dragonball_gracze on p.od = gracz where p.do = ".$uzytkownik['gracz']." and typ = 1 order by p.status asc, p.id desc" );
		...
		mysql_query("update dragonball_poczta set status = 1 where do =".$uzytkownik['gracz']." and typ = 1 and status = 0");
