	...
	switch($_GET['sparing']){
		case 1:
			...
			elseif($uzytkownik['zmeczenie'] < 10) echo "<p class='error'>Jesteś zbyt zmęczony</p><br class='clear'>";
				...
				$punkty_gracz = 10*$uzytkownik['sztuki_walki'] + 12*$uzytkownik['koncentracja'] + 8*$uzytkownik['szybkosc'] + 11*$uzytkownik['sila_ducha'] + 15*$uzytkownik['kondycja'];

				$punkty_przeciwnik = 10* rand(1,5) + 12* rand(1,5) + 8* rand(1,5) + 11* rand(1,5) + 15* rand(1,5);
				
				if($punkty_gracz > $punkty_przeciwnik) {
					mysql_query("update dragonball_gracze set akcje = akcje - 5, zmeczenie = zmeczenie - 10, kasa = kasa + ".($punkty_przeciwnik*10)." where gracz = ".$uzytkownik['gracz']); 
					echo "
					...
					mysql_query("update dragonball_gracze set akcje = akcje - 5, zmeczenie = zmeczenie - 10, kasa = kasa + ".($punkty_przeciwnik)." where gracz = ".$uzytkownik['gracz']);
					echo "
		...
		case 2:
			...
			elseif($uzytkownik['zmeczenie'] < 20) echo "<p class='error'>Jesteś zbyt zmęczony</p><br class='clear'>";
				...
				if($punkty_gracz > $punkty_przeciwnik) {
					mysql_query("update dragonball_gracze set akcje = akcje - 10, zmeczenie = zmeczenie - 20, kasa = kasa + ".($punkty_przeciwnik*10)." where gracz = ".$uzytkownik['gracz']); 
					echo "
					<p class='note'>Wygrałeś sparing</p><br class='clear'>
					<p>Wygrałeś swój sparing! Odbierasz wynagrodzenie w wysokości ".($punkty_przeciwnik*10)."$.</p>
					";
					...
					mysql_query("update dragonball_gracze set akcje = akcje - 10, zmeczenie = zmeczenie - 20, kasa = kasa + ".floor($punkty_przeciwnik/2)." where gracz = ".$uzytkownik['gracz']);
					echo "
					<p class='error'>Przegrałeś sparing</p><br class='clear'>
		...
		case 3:
			...
			elseif($uzytkownik['zmeczenie'] < 25) echo "<p class='error'>Jesteś zbyt zmęczony</p><br class='clear'>";
				...
				if($punkty_gracz > $punkty_przeciwnik) {
					mysql_query("update dragonball_gracze set akcje = akcje - 15, zmeczenie = zmeczenie - 25, kasa = kasa + ".($punkty_przeciwnik*10)." where gracz = ".$uzytkownik['gracz']); 
					echo "
					<p class='note'>Wygrałeś sparing</p><br class='clear'>
					<p>Wygrałeś swój sparing! Odbierasz wynagrodzenie w wysokości ".($punkty_przeciwnik*10)."$.</p>
					";
					...
					mysql_query("update dragonball_gracze set akcje = akcje - 15, zmeczenie = zmeczenie - 25, kasa = kasa + ".floor($punkty_przeciwnik/4)." where gracz = ".$uzytkownik['gracz']);
					echo "
					<p class='error'>Przegrałeś sparing</p><br class='clear'>
