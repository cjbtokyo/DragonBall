...
if(!empty($_POST['kod'])){
	...
	$jest = mysql_fetch_array(mysql_query("select id from dragonball_kody where kod = '".$kod."' and status = 0"));


	if(empty($jest)) echo "<p class='error'>Nie ma takiego kodu</p><br class='clear'>"; else {
		if($uzytkownik['vip'] > time()){
			mysql_query("update dragonball_gracze set vip = vip + ".(30*86400).", akcje_max = 150 where gracz = ".$uzytkownik['gracz']);
			$uzytkownik['vip'] += 30*86400;
			$uzytkownik['akcje_max'] = 150;
		} else {
			mysql_query("update dragonball_gracze set vip = ".(time() + 30*86400).", akcje_max = 150 where gracz = ".$uzytkownik['gracz']);
			$uzytkownik['vip'] = time() + 30*86400;
			$uzytkownik['akcje_max'] = 150;
			$uzytkownik['akcje_max'] += 50;
		}
		
		mysql_query("update dragonball_kody set status = 1, gracz_id = ".$uzytkownik['gracz']." where id = ".$jest['id']);
