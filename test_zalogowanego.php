...
if(empty($_SESSION['user'])){
    ...
    $uzytkownik = mysql_fetch_array(mysql_query("select *, (select count(*) from dragonball_poczta where typ = 1 and do = gracz and status = 0) as poczta from dragonball_gracze where gracz = ".$_SESSION['user']));
	...
	if(($uzytkownik['vip'] > 0) && ($uzytkownik['vip'] < time())){
		mysql_query("update dragonball_gracze set vip = 0, akcje_max = 100 where gracz = ".$uzytkownik['gracz']);
		$uzytkownik['vip'] = 0;
