...
if(!empty($_GET['ban'])){
	...
	mysql_query("update dragonball_gracze set zbanowany = 1 where gracz = ".$_GET['ban']);
	echo "<p class='note'>zbanowano gracza</p><br class='clear'>";
}elseif(!empty($_GET['unban'])){
	$_GET['unban'] = (int)$_GET['unban'];
	mysql_query("update dragonball_gracze set zbanowany = 0 where gracz = ".$_GET['unban']);
...
";
$gracze = mysql_query("select * from dragonball_gracze  order by zbanowany desc");
