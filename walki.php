...
if(!empty($_GET['walcz'])){
	...
	if($_GET['walcz'] == $uzytkownik['gracz']){
		...
		$vs = mysql_fetch_array(mysql_query("select * from dragonball_gracze where gracz = ".$_GET['walcz']));
		if(empty($vs)){
			...
			$punkty_przeciwnik = 10*$vs['sztuki_walki'] + 12*$vs['koncentracja'] + 8*$vs['szybkosc'] + 11*$vs['sila_ducha'] + 15*$vs['kondycja'];
			
			
			$pozycja = $uzytkownik['ranking'];
			if($punkty_gracz > $punkty_przeciwnik) {
				if($uzytkownik['ranking'] <= $vs['ranking']){
					mysql_query("update dragonball_gracze set akcje = akcje - 50, kasa = kasa + ".($punkty_przeciwnik*10)." where gracz = ".$uzytkownik['gracz']); 
					echo "
					...
					mysql_query("insert into dragonball_poczta (od, do, typ, tytul, tresc, data)
					...
					mysql_query("update dragonball_gracze set akcje = akcje - 50, kasa = kasa + ".($punkty_przeciwnik*50).", ranking = ".$vs['ranking']." where gracz = ".$uzytkownik['gracz']);
					
					mysql_query("update dragonball_gracze set  ranking = ".$uzytkownik['ranking']." where gracz = ".$vs['gracz']);
				...
				echo "
					...
					<p>Nie zdobywasz nic i odchodzisz.</p>
				";
				$uzytkownik['akcje'] -= 50;
...
$przeciwnicy = mysql_query("select * from dragonball_gracze where ranking <= ".$uzytkownik['ranking']." and gracz  != ".$uzytkownik['gracz']." limit 5");
