...
if(!empty($_POST['pkt'])){
	...
	elseif($_POST['pkt'] * 25 > $uzytkownik['kasa']) 
		...
		mysql_query("update dragonball_gracze set zmeczenie = zmeczenie + ".$_POST['pkt'].", kasa = kasa - ".($_POST['pkt'] * 25)." where gracz = ".$uzytkownik['gracz']);
...
if($uzytkownik['zmeczenie'] < $uzytkownik['zmeczenie_max']){
	echo"
		Jeżeli jesteś zmęczony, to w domu możesz odzyskać punkty zmęczenia.<br/>
