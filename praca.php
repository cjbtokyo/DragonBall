...
if(($uzytkownik['pracuje'] < time())  && ($uzytkownik['pracuje'] > 0)){
	mysql_query("update dragonball_gracze set pracuje = 0, pracuje_godzin = 0, kasa = kasa + ".($uzytkownik['pracuje_godzin'] * 500)." where gracz =".$uzytkownik['gracz']);
	header("location: praca.php");
}

if(isset($_GET['przerwij']) && ($uzytkownik['pracuje'] > 0)){
	mysql_query("update dragonball_gracze set pracuje = 0, pracuje_godzin = 0 where gracz =".$uzytkownik['gracz']);
	header("location: praca.php");
}
...
if(!empty($_GET['praca']) && ($uzytkownik['pracuje'] == 0)){
	switch($_GET['praca']){
		case 1:
			mysql_query("update dragonball_gracze set pracuje = ".(time() + 3600).", pracuje_godzin = 1 where gracz =".$uzytkownik['gracz']);
			header("location: praca.php");
		break;
		case 2:
			mysql_query("update dragonball_gracze set pracuje = ".(time() + 7200).", pracuje_godzin = 2 where gracz =".$uzytkownik['gracz']);
			header("location: praca.php");
		break;
		case 3:
			mysql_query("update dragonball_gracze set pracuje = ".(time() + 10800).", pracuje_godzin = 3 where gracz =".$uzytkownik['gracz']);
			header("location: praca.php");
		break;
		default:
