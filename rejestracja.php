...
if(!empty($_POST)){
    ...
    if (!empty($_POST['login']) && !empty($_POST['haslo']) && !empty($_POST['haslo2']) && ($_POST['haslo'] == $_POST['haslo2']) && !empty($_POST['email'])){
        ...
        if(strlen($_POST['login']) <5 ) {
            ...
            $zajety = mysql_fetch_array(mysql_query("select count(*) as blad from dragonball_gracze where login = '".$_POST['login']."' or email ='".$_POST['email']."'    "));
            ...
            if(empty($zajety)) {
                ...
                mysql_query("insert into dragonball_gracze (login, haslo, email, ranking) select                '".$_POST['login']."','".$_POST['haslo']."','".$_POST['email']."', count(*) + 1 from dragonball_gracze");
